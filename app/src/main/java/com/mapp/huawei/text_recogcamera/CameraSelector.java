package com.mapp.huawei.text_recogcamera;
...
public class CameraSelector {
    ...
    public Camera createCamera() throws IOException {
        int cameraId = CameraSelector.getIdForRequestedCamera(CameraConfiguration.getCameraFacingBack());
        if (cameraId == -1) {
    ...
    public int getFacing() {
        return CameraConfiguration.getCameraFacingBack();
    }
    ...
    private void setRotation(Camera camera, Camera.Parameters parameters, int cameraId) {
        if (this.activity != null){
            windowManager = (WindowManager) this.activity.getSystemService(Context.WINDOW_SERVICE);
        }
        ...
        int mRotation = windowManager.getDefaultDisplay().getRotation();
        int degrees = 0;
        switch (mRotation) {
